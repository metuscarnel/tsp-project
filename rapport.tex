...
Ce projet de cours d'Intelligence Artificielle s'insère dans la mise en application des notions théoriques acquises au cours du semestre. Il s'agit d'une étude et d'une implémentation d'un algorithme génétique pour résoudre le problème du voyageur de commerce (TSP).
...
\begin{itemize}
    ...
    \item \textbf{Fonction d'évaluation (fitness) :} La distance totale $D$ du circuit, calculée en utilisant la formule de Haversine, qui prend en compte la courbure de la Terre pour calculer les distances entre les villes à partir de leurs coordonnées géographiques. Entre deux solutions, celle qui a la plus petite distance totale est considérée comme la meilleure.
    $D$ s'exprime comme suit :
    \[D = \sum_{i=1}^{n-1} d(v_i, v_{i+1}) + d(v_n, v_1)\]
    où $d(v_i, v_j)$ est la distance entre les villes $v_i$ et $v_j$ calculée par la formule de Haversine et $v$ un individu.
\end{itemize}
...
\texttt{NB :} notons que la gestion de l'affichage est réalisée dans un fichier tiers pour séparer la logique de notre algorithme de la partie interface utilisateur. Pour ce faire, les bibliothèques suivantes ont été utilisées :
\begin{itemize}
...
Notre algorithme principal fonctionne suivant l'algorithme représenté par le pseudo-code ci-dessous :
\begin{algorithm}
...
La recherche de la solution optimale par une approche de force brute est représentée par le pseudo-code ci-dessous :
\begin{algorithm}
...
\begin{itemize}
    ...
    \item Utiliser la bibliothèque TSPLIB pour des benchmarks standardisés et comparer les performances de notre algorithme génétique avec d'autres approches publiées.
...
\bibliographystyle{plain} % Ajout d'un style de bibliographie standard si absent
\end{document}